...
export class IntegrationAccountAgreementTreeItem implements IAzureTreeItem {
	...
	public get agreementType(): AgreementType {
		return (AgreementType as any)[
			this.integrationAccountAgreement.agreementType
		];
	}
	...
	public async update(agreementContent: string): Promise<string> {
		const agreement: IntegrationAccountAgreement =
			JSON.parse(agreementContent);

		const updatedAgreement =
			await this.client.integrationAccountAgreements.createOrUpdate(
				this.resourceGroupName,
				this.integrationAccountName,
				this.integrationAccountAgreementName,
				agreement,
